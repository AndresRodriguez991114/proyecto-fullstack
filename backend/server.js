...
// -------------------------------------------------------------
app.delete("/api/departamentos/:id", auth, async (req, res) => {
  try {
    const { id } = req.params;
...
app.put("/api/equipos/:id/cerrar", auth, async (req, res) => {
  ...
  try {
    ...
    // 2️⃣ Cambiar estado a Listo para envio
...
// -------------------------------------------------------------
// 🟢 LISTAR EQUIPOS PARA ENVIO
// -------------------------------------------------------------
app.get("/api/equipos/listos-envio", auth, async (req, res) => {
  try {
    const r = await pool.query(`
      SELECT 
        e.id,
